...
square=svm.SVC(C=C,kernel="poly",degree=2)
quadratic=svm.SVC(C=C,kernel="poly",degree=3)

# Using the given function to plot results
plot_results([linear.fit(X,y),square.fit(X,y),quadratic.fit(X,y)],["linear","square","quadratic"],X,y)

# b
# Preparing the data
linear=svm.SVC(coef0=1,C=C,kernel="linear")
square=svm.SVC(coef0=1,C=C,kernel="poly",degree=2)
quadratic=svm.SVC(coef0=1,C=C,kernel="poly",degree=3)
...
# c
# Preparing the data
rbf=svm.SVC(coef0=1,C=C,kernel="rbf",gamma=10)

for i in range(n):
    y[i+n]=np.random.choice([-1, 1],  p=[0.9, 0.1])

# Using the given function to plot results
plot_results([square.fit(X,y),rbf.fit(X,y)],["square","RBF"],X,y)
